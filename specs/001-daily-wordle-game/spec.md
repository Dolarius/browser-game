...
A returning player sees the daily word puzzle on the home screen and can make five-letter guesses until they win or use all allowed guesses.
...
1. **Given** a local profile exists, **When** the player opens the home screen, **Then** the centered daily puzzle board and compact input controls are immediately playable.
...
- **FR-002**: The system MUST accept a non-empty nickname and save it as a local player profile in the browser.
...
- **FR-005**: The system MUST use a bundled local word list for both daily answers and allowed guesses.
...
- **Local Player Profile**: A locally stored player identity containing the player's nickname and profile creation state.
...
- **Word List**: Bundled local collections of answer words and allowed guess words used without network access.
...
- The nickname is stored only on the current browser and device.
- The daily puzzle is based on the user's local calendar date.
- The bundled answer list is large enough to support daily play by cycling when needed.
